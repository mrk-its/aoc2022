...
fn tests_impl(args: TokenStream, input: TokenStream) -> parse::Result<TokenStream> {
    ...
    let krate = format_ident!("mos_test");
    ...
    for test in &tests {
        ...
        if ignore {
            ...
            unit_test_calls.push(quote!(
                #before_each_call;
                #krate::export::check_outcome(#krate::OutcomeWrapper(#call), #should_error);
                #after_each_call;
            ));
        }
    }
    ...
    let unit_test_progress = tests
        ...
        .map(|test| {
            ...
            quote_spanned! {
                test.func.sig.ident.span() => ufmt_stdio::println!(#message, __defmt_test_number, DEFMT_TEST_COUNT);
            }
        })
        .collect::<Vec<_>>();
    Ok(quote!(
    ...
    mod #ident {
        ...
        #[export_name = "main"]
        unsafe extern "C" fn __defmt_test_entry() -> isize {
            #declare_test_count
            #init_expr
            ...
            ufmt_stdio::println!("all tests passed!");
            // #krate::export::exit()
            0
        }
